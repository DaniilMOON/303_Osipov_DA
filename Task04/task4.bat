...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select title, year, case when  max_raiting = avg_raiting then 'Yes' else 'No' end as recommendation from(select m.title, m."year", round(avg(r.rating), 2) as avg_raiting, max(round(avg(r.rating), 2)) over() as max_raiting, min(round(avg(r.rating), 2)) over() as min_raiting from ratings r join movies m on r.movie_id = m.id group by r.movie_id) where avg_raiting = max_raiting or avg_raiting = min_raiting order by year, title;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "with recursive find_genres(genres, str) as ( select substr(m.genres || '|', 1, instr(m.genres || '|', '|') - 1), substr(m.genres || '|', instr(m.genres || '|', '|') + 1) from movies m union all select substr(str, 1, instr(str, '|') - 1), substr(str, instr(str, '|') + 1) from find_genres where str != '') select genres from find_genres group by genres;"
